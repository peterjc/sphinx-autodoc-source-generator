...
def make_module_text(py_file,root,package,subroot,master_package,text):
    ...
    if not opts.onemodule:
        for py_file in py_files:
            if shall_skip(os.path.join(root, py_file)):
                continue
            text = make_module_text(py_file,root,package,subroot,master_package,text)
    else:
        py_file = os.path.splitext(INIT)[0]
        text += format_directive(subroot or makename(subroot, py_file),master_package)
